...
import time, argparse, subprocess as sp, os, Bio.SeqIO
...
def parse_arguments():
	...
	parser.add_argument('-i', dest='in', type=str, required=True, metavar='PATH',
		help="""Input viral sequences in FASTA format""")
	parser.add_argument('-o', dest='out', type=str, required=True, metavar='PATH',
		help="""Output path""")
	parser.add_argument('--min_len', type=int, default=2000, metavar='INT',
		help="""Min contig length (2000)""")
	parser.add_argument('--min_dtr', type=int, default=20, metavar='INT',
		help="""Min length of DTRs (20)""")
	parser.add_argument('--max_lc', type=float, default=20.0, metavar='FLOAT',
		help="""Max %% of DTR classified as low complexity (20.0)""")
	parser.add_argument('--keep_rep', action='store_true', default=False,
...
def main():
	...
	dtr_path = '%s/dtr.fna' % args['tmp']
	with open(dtr_path, 'w') as f:
		for num in sorted(genomes.keys()):
			genome = genomes[num]
			if genome.dtr.length >= 10:
				f.write('>'+genome.num+'\n'+genome.dtr.seq+'\n')
	cmd = "dustmasker -in %s -out %s/dustmasker.txt -outfmt acclist" % (dtr_path, args['tmp'])
	p = sp.Popen(cmd, shell=True)
	p.wait()
	for line in open('%s/dustmasker.txt' % args['tmp']):
	...
	print("write summary...")
	header = ['genome_num', 'genome_id', 'genome_length', 'dtr_length', 'dtr_count', 'dtr_dust', 'is_complete']
	...
	for genome_num in sorted(genomes.keys()):
		...
		dtr = genome.dtr
		is_complete = 'No'
		if (genome.length >= args['min_len']
				and dtr.length >= args['min_dtr']
				and (dtr.count == 2 or args['keep_rep'])
				and (100.0*dtr.dust/dtr.length <= args['max_lc'])):
			is_complete = 'Yes'
		row = [genome_num, genome.id, genome.length, dtr.length, dtr.count, dtr.dust, is_complete]
